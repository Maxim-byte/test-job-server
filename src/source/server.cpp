...
server::server(boost::asio::io_context &io, boost::asio::ip::tcp::endpoint &&endpoint) :
    ...
    if (ec) {
        throw std::runtime_error("can't open acceptor!");
    }
    acceptor_.set_option(boost::asio::socket_base::reuse_address(true), ec);

    if (ec) {
        throw std::runtime_error("can't set reusing address!");
    }
    acceptor_.bind(endpoint_, ec);
    if (ec) {
        throw std::runtime_error("can't bind acceptor!");
    }
    acceptor_.listen(boost::asio::socket_base::max_listen_connections, ec);
    if (ec) {
        throw std::runtime_error("can't listen using acceptor!");
    }
}
